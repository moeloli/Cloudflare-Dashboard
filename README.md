...
本项目的「一键加速」与「SaaS 优选」功能会部署回源 Worker / 配置 Cloudflare for SaaS 回退源，本质是为**你自己拥有合法权限的源站**做边缘加速。请务必合规使用：

- **仅用于自有或已获授权的源站**。不得用于为赌博、钓鱼、诈骗、侵权、色情等违法或违规站点提供加速、隐匿源站或规避溯源。
- **不得利用优选域名 / SaaS custom hostname 规避 Cloudflare 的滥用检测与封禁**。本功能基于 Cloudflare 服务，须遵守 [Cloudflare 自我服务订阅协议](https://www.cloudflare.com/terms/) 与可接受使用政策（AUP）。
- 加速的访问域名、回源域名、源站内容**由部署者自负其责**。因滥用导致的账号封禁、服务终止、法律责任，与本项目作者无关。
- 一旦发现被用于违法用途，请立即停止使用并清除相关配置（Worker 脚本、custom hostname、回退源、DNS 记录）。

> 本项目仅提供技术工具，不提供、不运营任何优选节点或 CDN 服务，不对使用者的源站内容进行审查。请在法律与各服务商政策允许的范围内使用。
